...
/**
 * 2Moons - Modernized Installer
 * 
 * Modernized installer with Bootstrap 5, Anime.js animations,
 * futuristic UI design, and comprehensive system checks.
 *
 * @package 2Moons
...
$template->assign([
    ...
    'title' => $LNG['title_install'] . ' &bull; 2Moons Futuristic',
...
// Auto-enable install tool if first install file exists
if (is_file($quickStartFile) && is_writeable($quickStartFile) && unlink($quickStartFile)) {
    @touch($enableInstallToolFile);
}

// Clean up old install tool files
        ...
        switch ($step) {
            ...
            case 2:
                ...
                if (version_compare(PHP_VERSION, "8.3.0", ">=")) {
                    $PHP = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . ", v" . PHP_VERSION . "</span>";
                } else {
                    $PHP = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . ", v" . PHP_VERSION . "</span>";
                    $error = true;
                }

                // PDO Check
                if (class_exists('PDO') && in_array('mysql', PDO::getAvailableDrivers())) {
                    $pdo = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . "</span>";
                } else {
                    $pdo = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . "</span>";
                    $error = true;
                }

                // JSON Check
                if (function_exists('json_encode')) {
                    $json = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . "</span>";
                } else {
                    $json = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . "</span>";
                    $error = true;
                }

                // ini_set Check
                if (function_exists('ini_set')) {
                    $iniset = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . "</span>";
                } else {
                    $iniset = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . "</span>";
                    $error = true;
                }

                // Register Globals Check
                if (!ini_get('register_globals')) {
                    $global = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . "</span>";
                } else {
                    $global = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . "</span>";
                    $error = true;
                }

                // GD Library Check
                if (!extension_loaded('gd')) {
                    $gdlib = "<span class=\"badge bg-danger\">" . $LNG['reg_no'] . "</span>";
                } else {
                    $gdVersion = '0.0.0';
                    ...
                    $gdlib = "<span class=\"badge bg-success\">" . $LNG['reg_yes'] . ", v" . $gdVersion . "</span>";
                }
                ...
                if (file_exists(ROOT_PATH . "includes/config.php") || @touch(ROOT_PATH . "includes/config.php")) {
                    if (is_writable(ROOT_PATH . "includes/config.php")) {
                        $chmod = "<span class=\"badge bg-success\"> - " . $LNG['reg_writable'] . "</span>";
                    } else {
                        $chmod = " - <span class=\"badge bg-danger\">" . $LNG['reg_not_writable'] . "</span>";
                        $error = true;
                        $ftp = true;
                    }
                    $config = "<tr><td class=\"transparent left\"><p>" . sprintf($LNG['reg_file'], 'includes/config.php') . "</p></td><td class=\"transparent\"><span class=\"badge bg-success\">" . $LNG['reg_found'] . "</span>" . $chmod . "</td></tr>";
                } else {
                    $config = "<tr><td class=\"transparent left\"><p>" . sprintf($LNG['reg_file'], 'includes/config.php') . "</p></td><td class=\"transparent\"><span class=\"badge bg-danger\">" . $LNG['reg_not_found'] . "</span></td></tr>";
                    $error = true;
                    $ftp = true;
                }
                ...
                foreach ($directories as $dir) {
                    if (file_exists(ROOT_PATH . $dir) || @mkdir(ROOT_PATH . $dir)) {
                        ...
                        $dirs .= "<tr><td class=\"transparent left\"><p>" . sprintf($LNG['reg_dir'], $dir) . "</p></td><td class=\"transparent\"><span class=\"badge bg-success\">" . $LNG['reg_found'] . "</span>" . $chmod . "</td></tr>";
                    } else {
                        $dirs .= "<tr><td class=\"transparent left\"><p>" . sprintf($LNG['reg_dir'], $dir) . "</p></td><td class=\"transparent\"><span class=\"badge bg-danger\">" . $LNG['reg_not_found'] . "</span></td></tr>";
                    }
                }

                if ($error == false) {
                    $done = '<tr class="noborder"><td colspan="2" class="transparent"><a href="index.php?mode=install&step=3"><button class="btn btn-primary btn-lg" style="cursor: pointer;">' . $LNG['continue'] . '</button></a></td></tr>';
                } else {
                    $done = '';
                }
